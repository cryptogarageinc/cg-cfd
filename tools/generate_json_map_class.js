...
// ----------------------------------------------------------------------------
let fileList = [];
const folderPath = `${__dirname}/../../cfd/src/input_json_format/`;
const outJsonFolderPath = `${__dirname}/output/`;
const outStructDirPath = `${__dirname}/../../cfd/include/cfd/`;
const outStructFileName = `cfdapi_struct.h`;
let jsonDataList = [];

if (fs.existsSync(outJsonFolderPath) && fs.statSync(outJsonFolderPath).isDirectory()) {
  // do nothing
} else {
  fs.mkdirSync(outJsonFolderPath);
}
